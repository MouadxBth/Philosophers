...
/*   Updated: 2022/10/12 16:32:41 by mbouthai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_validate_info(t_info *info, int argc)
{
	if (!info || info->number_of_philosophers <= 0)
		return (printf("[Error]: Number of philosophers cannot be less or equal to 0."), 0);
	if (info->time_to_die < 0)
		return (printf("[Error]: Time to die cannot be less than 0."), 0);
	if (info->time_to_eat < 0)
		return (printf("[Error]: Time to eat cannot be less than 0."), 0);
	if (info->time_to_sleep < 0)
		return (printf("[Error]: Time to sleep cannot be less than 0."), 0);
	if (argc - 1 == 5 && info->minimum_eat_times <= 0)
		return (printf("[Error]: Minimum eating times cannot be less or equal than 0."), 0);
	return (1);
}
